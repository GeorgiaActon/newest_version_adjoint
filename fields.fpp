...
!        deallocate (g0)
   ...
   subroutine get_adjoint_fields_vmulo(g, phi)
      ...
      use dist_fn_arrays, only: star_ad_field !wstar
...
!     complex, dimension (:,:,:), allocatable :: star
      ...
      ! if(.not. allocated(star)) then
      !    allocate(star(nalpha,-nzgrid:nzgrid,vmu_lo%llim_proc:vmu_lo%ulim_alloc))
      !    star = 0.
      ! end if
     !! Adjoint - Calculate omega* frequency with the correct sign

!     star = -wstar/code_dt

      allocate (eta(naky, nakx, -nzgrid:nzgrid)); eta = 0.

     !! Adjoint - equivalent of 'gamtot' for adjoint equatio
     !!         - sum over species of Z^2*n/T int(F_0)
      ia = 1
      allocate (g0(nvpa, nmu))
      do ikxkyz = kxkyz_lo%llim_proc, kxkyz_lo%ulim_proc
         it = it_idx(kxkyz_lo, ikxkyz)
     ...
     !! Adjoint - chi = sum int( (i* omega_star - Z/T *omega_g*F_0 )* J0*lambda / eta
      ...
      do it = 1, ntubes
         do iz = -nzgrid, nzgrid
            do ivmu = vmu_lo%llim_proc, vmu_lo%ulim_proc
               ...
               ! zi*spread(aky,2,nakx)*star(ia,iz,ivmu)/spec(is)%zt - omega_g(:,:)* &
